...
class FaceMap3DMMDetector:
    ...
    def detect(
        ...
        # Transform from resized crop space to original frame space
        crop_h = y2 - y1
        crop_w = x2 - x1
        landmark[:, 0] = (
            (landmark[:, 0] + self.INPUT_SIZE / 2) * crop_w / self.INPUT_SIZE + x1
        )
        landmark[:, 1] = (
            (landmark[:, 1] + self.INPUT_SIZE / 2) * crop_h / self.INPUT_SIZE + y1
        )
